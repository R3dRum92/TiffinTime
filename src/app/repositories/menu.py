...
async def get_all_menus_by_vendor(
    ...
    try:
        response = await client.table("menu_items")\
            .select("*")\
        ...
        for item in response.data:
            ...
            if item.get("img_bucket") and item.get("img_path"):
                try:
                    url_response = await client.storage.from_(item["img_bucket"]).create_signed_url(
                        path=item["img_path"],
                        expires_in=3600,
                    )
                    img_url = url_response.get("signedURL")
                except Exception as e:
                    logger.warning(f"Failed to create signed URL for {item.get('id')}: {e}")
...
async def upload_menu_image(
    ...
    try:
        item = await client.table("menu_items")\
            .select("*")\
            .eq("id", str(item_id))\
            .eq("vendor_id", str(vendor_id))\
            .single()\
    ...
    try:
        ...
        # Update database
        update_response = await client.table("menu_items")\
            .update({
...
async def get_vendor_menu_with_availability(
    ...
    # Check date_specials
    specials = await client.table("date_specials")\
        .select("menu_item_id")\
        .eq("available_date", today)\
        .execute()
    ...
    weekly = await client.table("weekly_availability")\
        .select("menu_item_id")\
