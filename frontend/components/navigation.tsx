...
//   const shouldShowNavigation = allowedPages.some(page => {
//     if (page === '/') {
//       return pathname === '/' // Exact match for home page
//     }
//     return pathname.startsWith(page) // Starts with match for other pages
//   })
...
//     }
//     return pathname.startsWith(path) // Starts with match for other pages
//   }
...
import { useState, useEffect } from 'react'
import { usePathname } from 'next/navigation'
import { useUser } from '@/app/hooks/getUserDetails' // Adjust path as needed
        ...
        <div className="hidden md:flex items-center space-x-4">
          {isAuthenticated ? (
            <div className="flex items-center space-x-4">
              <span className="darktext font-medium">Welcome, {user.name}</span>
              <button 
                onClick={handleLogout}
                className="bgtheme text-white font-bold px-6 py-2 rounded-full hover:bg-red-500 hover:scale-105 hover:shadow-lg transition-all duration-300 ease-in-out transform cursor-pointer"
              >
                Logout
              </button>
            </div>
              ...
              {isAuthenticated ? (
                <div className="space-y-3">
                  <div className="text-center">
                    <span className="darktext font-medium text-sm">Welcome, {user.name}</span>
                  </div>
                  <button 
                    onClick={handleLogout}
                    className="w-full bgtheme text-white font-bold px-6 py-2 rounded-full hover:bg-red-500 transition-colors"
                  >
                    Logout
                  </button>
                </div>
